...
class WebsocketHandler(tornado.websocket.WebSocketHandler):
    ...
    def select_subprotocol(self, subprotocols):
        ...
        protocols = mem.botrock.digest_protocol( subprotocols )
    ...
    def open(self):
        ...
        protocolElements = mem.botrock.digest_protocol(self.request.headers.get('Sec-Websocket-Protocol', []))
        ...
        # add to connections
        mem.botrock.add_connection(connection_id=connection_id, connection=self, client=client)
    ...
    def on_message(self, message):
        ...
        if 'method' in message:
            ...
            if hasattr(mem.botrock, message['method']):
                getattr(mem.botrock, message['method'])(data=data, callback=lambda response, error=False: self.handle_response(response=response, error=error, request_id=request_id))
    ...
    def on_close(self):
        mem.botrock.remove_connection(connection_id=self.connection_id)
    ...
    def handle_response(self, *args, **kwargs):
        ...
        if error:
            ...
            #mem.botrock.raven_client.captureMessage(data.message)
        ...
        mem.botrock.send_message(connection_id=self.connection_id, data=data)
...
class HttpHandler(tornado.web.RequestHandler):
    ...
    def get(self, slug=None):
        ...
        # make sure the method exists
        if hasattr(mem.botrock, slug):
            getattr(mem.botrock, slug)(request=self.request, callback=lambda response, error=False: self.handle_response(response=response, error=error))
    ...
    def post(self, slug=None):
        ...
        if hasattr(mem.botrock, slug):
            try:
                ...
                getattr(mem.botrock, slug)(data=data, request=self.request, callback=lambda response, error=False: self.handle_response(response=response, error=error))
